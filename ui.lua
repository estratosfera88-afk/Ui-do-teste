-- [[ AKATSUKI UI ONLY [v6.1 - FIXES APLICADOS] - REFINED UNIFIED EDITION ]]
-- Fixes v6.1:
--   - Títulos dos toggles em inglês, descrições em português
--   - Pergunta de fechamento em português
--   - TpMap removido; TpMurder e TpSheriff adicionados na aba Teleports
--   - Value label do slider movido para a direita (sem sobrepor a track)
--   - Botão AUTO SHOOT flutuante chama AutoShootOnce a cada clique
...
-- ==================== TEXTOS DA UI ====================
-- FIX UI 1: Títulos em inglês, descrições em português, fechamento em português
local UI_TEXT = {
	...
	Options           = {
		AutoShoot    = { Title = "Auto Shoot",    Desc = "Equipa a arma automaticamente e atira no Assassino detectado usando Silent Aim." },
...
local function GetFloatingInitialPosition(side)
	...
	local centerX = (side == "right")
		and (mainPos.X + mainSize.X + FLOATING_GAP + halfW)
		or  (mainPos.X - FLOATING_GAP - halfW)
	local safePos = ClampFloatingToSafeArea(Vector2.new(centerX, centerY))
	return UDim2.fromOffset(safePos.X, safePos.Y)
end
...
UserInputService.InputChanged:Connect(function(input)
	...
	local shouldMove = (state.inputType == Enum.UserInputType.MouseButton1)
	...
	local safePos = ClampFloatingToSafeArea(Vector2.new(startX + delta.X, startY + delta.Y))
	if state.root and state.root.Parent then
		state.root.Position = UDim2.fromOffset(safePos.X, safePos.Y)
	end
end)
...
local function SyncFloatingButton(configKey)
	if configKey == "AutoShoot" then
		if Configs[configKey] then
			-- FIX UI AUTO SHOOT: cada toque chama AutoShootOnce, não toggle.
...
_G.AkatUIShutdown = function()
	...
	for key, data in pairs(floatingButtons) do
		if data and data.root then
			pcall(function() if data.root.Parent then data.root:Destroy() end end)
		end
...
local function CriarNotificacao(titulo, descricao)
	...
	local function DismissNotif()
		...
		TweenService:Create(notifScale,  TweenInfo.new(0.22), {Scale = 0.96}):Play()
...
local function CriarNotificacaoDiscord()
	...
	copyBtn.MouseButton1Click:Connect(function()
		PlayUI_Click()
		pcall(function() if setclipboard then setclipboard("https://discord.gg/rZuYzZ7zvt") end end)
	end)
	...
	local function DismissNotif()
		...
		UpdateNotifications()
		TweenService:Create(notifHolder, TweenInfo.new(0.25, Enum.EasingStyle.Cubic, Enum.EasingDirection.In), {Position = UDim2.new(1, 360, notifHolder.Position.Y.Scale, notifHolder.Position.Y.Offset)}):Play()
		TweenService:Create(notifScale,  TweenInfo.new(0.22), {Scale = 0.96}):Play()
		task.delay(0.28, function() if notifHolder and notifHolder.Parent then notifHolder:Destroy() end end)
	end
			...
			if shouldBeVisible then
				...
				task.delay(delay, function()
					...
					TweenService:Create(child, TweenInfo.new(0.2, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
						Size = UDim2.new(1, -10, 0, child:GetAttribute("ItemHeight") or 60),
...
local function createToggle(parent, configKey, tabCategory)
	...
	triggerBtn.MouseButton1Click:Connect(function()
		...
		TweenService:Create(switchCircle, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = on and UDim2.new(1, -20, 0.5, -9) or UDim2.new(0, 2, 0.5, -9)}):Play()
		TweenService:Create(switchTrack,  TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = on and Color3.fromHex("#8B0000") or Color3.fromRGB(30, 30, 30)}):Play()
		...
		if _G.AkatCallbacks and type(_G.AkatCallbacks[configKey]) == "function" then
			local success, err = pcall(function() _G.AkatCallbacks[configKey](on) end)
			if not success then warn("[AKAT UI] Callback failed for " .. tostring(configKey) .. ": " .. tostring(err)) end
		end
		SyncFloatingButton(configKey)
	end)
end

-- ==================== TOGGLE COMPACTO (sem descrição) ====================
local function createCompactToggle(parent, configKey, tabCategory)
	...
	hit.MouseButton1Click:Connect(function()
		...
		Configs[configKey] = not Configs[configKey]
		local on = Configs[configKey]
		TweenService:Create(circle, TweenInfo.new(0.22), {Position = on and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)}):Play()
		TweenService:Create(track,  TweenInfo.new(0.22), {BackgroundColor3 = on and Color3.fromHex("#8B0000") or Color3.fromRGB(30, 30, 30)}):Play()
		if _G.AkatCallbacks and type(_G.AkatCallbacks[configKey]) == "function" then
...
-- ==================== FIX UI 2: SLIDER — VALUE LABEL MAIS À DIREITA ====================
-- O value label foi movido para FORA da track (à direita dela),
-- eliminando o problema de sobreposição com a linha do slider.
...
local function ApplyResponsiveWindowSize(animate)
	...
	task.defer(function() ClampMainWrapperToViewport() end)
end
...
SetUIState = function(newState)
	...
	if newState == "OPEN" then
		...
		local targetSize = isExpanded and select(2, GetResponsiveUISizes()) or select(1, GetResponsiveUISizes())
	...
	elseif newState == "MINIMIZED" or newState == "CLOSED" then
		...
		local vp = GetViewportSize()
		local closeTween = TweenService:Create(mainWrapper, windowAnim, {
...
local function AplicarClickFlash(btn, icon)
	btn.MouseButton1Click:Connect(function()
		TweenService:Create(icon, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, true), {ImageColor3 = Color3.fromRGB(255, 255, 255)}):Play()
	end)
end
...
local function AlternarConfirmacao(exibir)
	...
	if exibir then
		...
		TweenService:Create(confirmBlur, TweenInfo.new(tempoAnim), {Size = 28}):Play()
		...
		TweenService:Create(cardScale, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Scale = 1}):Play()
		...
		if sc then TweenService:Create(sc, TweenInfo.new(0.18), {Scale = 0.88}):Play() end
...
-- FIX UI 3: Teleports — sem TpMap, com TpMurder e TpSheriff
...
local function ExecutarIntroAkat()
	...
	TweenService:Create(IntroFrame, TweenInfo.new(0.5), {BackgroundTransparency = 0.05}):Play()
	TweenService:Create(Blur, TweenInfo.new(0.5), {Size = 24}):Play()
	task.wait(0.1)
	TweenService:Create(IntroText, TweenInfo.new(0.85, Enum.EasingStyle.Quart), {Position = UDim2.new(0, 0, 0, 0)}):Play()
	task.wait(0.2)
	TweenService:Create(IntroLine, TweenInfo.new(0.7), {BackgroundTransparency = 0, Size = UDim2.new(0, 260, 0, 2)}):Play()
	task.wait(1.6)
	TweenService:Create(IntroText, TweenInfo.new(0.4), {TextTransparency = 1}):Play()
	TweenService:Create(IntroLine, TweenInfo.new(0.4), {Size = UDim2.new(0, 0, 0, 2), BackgroundTransparency = 1}):Play()
	task.wait(0.3)
	...
	TweenService:Create(FloatBtn, TweenInfo.new(0.5, Enum.EasingStyle.Back), {Size = UDim2.new(0, 44, 0, 44)}):Play()

	CriarNotificacao("AKATSUKI SCRIPTS", "MM2 Script iniciado com sucesso. Bem-vindo, " .. player.DisplayName .. ".")
	...
	openScale.Completed:Connect(function()
		...
		IntroFrame:Destroy()
		task.defer(function() selectTab("Player") end)
	end)
end
